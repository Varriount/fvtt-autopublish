...
def main(
    ...
    **kwargs
    ...
    if manifest_url is not None:
        ...
        for form_key, manifest_key in FORM_KEY_TO_MANIFEST_KEY_MAP.items():
            version_data.setdefault(form_key, manifest_data[manifest_key])
    ...
    # Select the module versions form.
    br.select_form(id='package_form')
    fill_out_version_form(br, version_data)
    br.submit()
...
def fill_out_version_form(br, version_data: Dict):
    ...
    def field_name_for(name: str, version_index):
        return f'versions-{version_index}-{name}'


    def current_value_for(name: str):
    ...
    for name, value in version_data.items():
        field_name = field_name_for(
            name          = name,
